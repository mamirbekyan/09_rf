...
First, let's specify:  
  ...
  - which **engine** /package we want to use to run random forest (ranger)
...
rf_spec <- 
  ...
  rand_forest(trees = tune(),#tune because we dont know how many we want
              mtry = tune()
              ) %>% 
    # Specify the engine
  set_engine("ranger") %>% #we dont need to take engine=specific hyperparameters and take an extra step like in CIT
    # Specifying mode  
set_mode("regression")
...
resampling_foldcv  #we have 5 folds, can look into any of them
...
best_rmse <- rf_grid_result %>%
  select_by_pct_loss("trees", #the hyperparametre level we want
                     metric = "rmse",
                     limit = 2 #i accept a 2% loss in rmse when selecting the best rmes
                     )
...
```{r final_spec}
final_spec <- rand_forest(trees = best_r2$trees, #replacing the tune() 
                          mtry = best_r2$mtry) %>%
  # Specify the engine
set_engine("ranger",
...
```{r}
final_fit %>%
  collect_metrics()  #this is the test set. It is lower than the training set, which the algorithm learned on
```
...
#this is the training data. Very high rsq
```
